...
public class PartController {
	...
	public PartController(PartsModel partsModel,
	...
	public void createPart(PartView pv) {
		...
		try {
			// create the new part
			newPart = new Part(pv.getPartNumber(), pv.getPartName(),
					pv.getVendor(), pv.getQuantityUnit(),
					pv.getExternalPartNumber());
		...
		if (pv.getPart() != null) { // if template
			...
			// reset the view to match the part's information because it was
			// changed while creating the template
			pv.syncWithPart();
		} else {
	...
	public void updatePart(PartView pv) {
		...
		try {
			...
			// check if another part has the same part number the user is
			...
			// update the part
			oldPart.updatePart(pv.getPartNumber(), pv.getPartName(),
					pv.getVendor(), pv.getQuantityUnit(),
					pv.getExternalPartNumber());
		} catch (Exception ex) {
			pv.showError(ex.getMessage());
