...
public class ProductTemplateController {
	...
	public void createProductTemplate(ProductTemplateView ptv) throws Exception {
		...
		ProductTemplate template = new ProductTemplate(ptv.getProductNumber(),
				ptv.getProductDescription());
	...
	public void updateProductTemplate(ProductTemplateView ptv) throws Exception {
		...
		if (!oldTemplate.getProductNumber().equals(ptv.getProductNumber()))
			if (templatesModel.templateExistsByProductNumber(ptv
					.getProductNumber()))
				throw new Exception(
						"Error: That product template already exists.");

		// update the product template in database
		Main.productTemplateGateway.updateProductTemplate(oldTemplate,
				ptv.getProductNumber(), ptv.getProductDescription());

		// update the template
		oldTemplate.updateTemplate(ptv.getProductNumber(),
				ptv.getProductDescription());
	...
	public void addProductTemplatePart(ProductTemplateView ptv)
			throws Exception {
		ProductTemplatePartView templatePartView = new ProductTemplatePartView(
				ptv);
		ptv.setRightPanel(templatePartView);
	}

	public void deleteProductTemplatePart(ProductTemplateView ptv)
			throws Exception {
		System.out
				.println("ProductTemplateController.deleteProductTemplatePart(): Delete");
		ProductTemplatePart tp = ptv.getSelectedPart();
		// TODO: delete from view list
		Main.productTemplateGateway.deleteProductTemplatePart(tp
				.getProductTemplateID().toString(), tp);
		ptv.removePartByPartID(tp.getPartID().toString());
	}

	public void createProductTemplatePart(ProductTemplate template,
			Part selectedPart, double quantity) throws Exception {
		if (selectedPart == null)
			throw new Exception("Error: You need to select a part.");
		if (template == null) {
		} else {
			ProductTemplatePart templatePart = new ProductTemplatePart(
					template.getTemplateID(), selectedPart.getItemID(),
					quantity);
	...
	public void updateProductTemplatePart(ProductTemplatePart templatePart,
			double quantity) throws Exception {
		if (templatePart == null)
			throw new Exception(
					"Error: That product template part does not exists.");
		Main.productTemplateGateway.updateProductTemplatePart(templatePart,
				quantity);
